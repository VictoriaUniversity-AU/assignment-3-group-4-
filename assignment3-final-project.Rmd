...
#Calculate no. of days between Scheduled date and appointment date. 
data <- data %>%
  mutate (days_lag=
            as.integer(
            difftime(as.POSIXct(data$AppointmentDay),
...
recipe_patients <- 
  recipe(No.show ~ 
           Age+  Scholarship+Hipertension+Diabetes+Alcoholism+Handcap+ SMS_received,days_lag,
           data = train_data) %>%
